...
3. When SiteGraph is active (`--enable-sitegraph`), run `sitegraph` action `sync` with `url_prefix`, inspect `sitegraph` action `stats`, cluster endpoints with `sitegraph` action `clusters`, and search parameter names with `sitegraph` action `search`. Note that `sitegraph_sync` automatically annotates interesting newly indexed HTTP Proxy history entries with medium+ findings using a stable Burp history ID, bounding to 50 entries per run, preserving existing operator notes and highlights, and setting color flags (critical/high -> RED, medium -> ORANGE).
...
- **Compound Automated Workflows**:
  - **IDOR Verification**: Use `burp_verify_idor` with two authorization contexts. A finding requires successful 2xx responses; denied/error bodies never confirm. Treat different successful responses as inconclusive unless the configured pattern or similarity threshold supplies evidence.
  - **CORS Audit**: Use `burp_check_cors` with a bounded `test_origins` list. Findings are derived from the response header block only, never body text; probe failures are explicit errors.
  - **Access Control Matrix**: Use `burp_auth_matrix` with bounded endpoints and roles. Every matrix cell must have a real HTTP response; unauthenticated successful access is reported as a potential violation.
  - **JWT Audit**: Use `burp_audit_jwt` with an exactly three-segment token. Each malicious vector returns HTTP status and response length; malformed tokens fail before sending.
  - **SSRF Verification**: Use `burp_verify_ssrf` with `header:NAME`, `param:NAME`, `body`, or a bare body parameter name. Only Collaborator interactions carrying a payload generated by the current run count as evidence.
  - **Blind SQLi Verification**: Use `burp_verify_sqli_blind`; `param_type` accepts `query` (default) or `body`, and existing query parameters are retained. Sleep and collection bounds are enforced.
  - **GraphQL Audit**: Use `burp_audit_graphql`; introspection, suggestions, and batching are detected from parsed JSON shapes, not substring matches.
  - **CSRF/SameSite Verification**: Use `burp_verify_csrf_samesite` only with a state-changing method. The tool sends a real request, inspects the named cookie's `Set-Cookie` SameSite attribute, and generates an escaped PoC; a denial is not a vulnerability confirmation.
  - **API Fuzzing**: Use `burp_api_fuzz_orchestrator` with a bounded OpenAPI/Swagger spec and category list. It reports completed requests and HTTP 5xx anomalies, and stops on transport/no-response failures.
