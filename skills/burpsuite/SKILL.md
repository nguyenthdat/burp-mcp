...
2. **Start Read-Only**: Prefer Proxy history (`burp_proxy_history`), Target site map (`burp_sitemap`), target info (`burp_target_info`), Scanner issues (`burp_scan_issues`), sitegraph queries (`sitegraph_search`), and offline decoder operations (`decoder`) before generating active traffic.
3. **Preserve Operator State**: Record every temporary scope addition, intercept state change, HTTP handler, proxy rule, session rule, macro, cookie, background job, and managed WebSocket connection. Restore or remove it during cleanup.
4. **Interception Discipline**: Do not enable proxy interception in unattended flows. Read `burp_intercept_state` before automated traffic; if enabled, request confirmation before disabling it and restore original state upon completion.
5. **Rate & Concurrency Bounds**: Limit parallel requests (at most 32 via `burp_send_request_parallel`), cap scan concurrency via custom resource pools (`burp_scan_pool_create`), and pace background job polling.
6. **Protect Secrets & Redact Evidence**: Never log or disclose private keys, session cookies, auth tokens, or Collaborator secrets in reports.
...
1. Search Proxy HTTP history with `burp_proxy_history` using URL, method, status code, notes, or highlight filters.
2. Inspect full request and response headers/bodies with `burp_proxy_detail`.
3. Extract tokens, nonces, or canary reflections from responses using `burp_extract_from_response`.
4. Review WebSocket frame history with `burp_proxy_websocket_history`.
5. Annotate or highlight critical requests with `burp_annotate` or `burp_highlight`.

### 4. Vulnerability Testing & Request Replay
...
- **Manual Request Crafting**:
  - Replay modified requests via `burp_send_request`.
...
- **Race Conditions**:
  - Test multi-threaded limit overruns or concurrency bugs using `burp_race_condition`.
- **Out-of-Band (OAST) & SSRF**:
  - Generate interaction identifiers with `burp_collaborator_generate`.
  - Inject identifiers into headers, callbacks, or parameters, then poll for DNS/HTTP interactions with `burp_collaborator_poll`.
- **Live WebSockets**:
...
### 5. Fuzzing & Payload Lists
1. **Bounded Request Fuzzing**: Use `burp_inline_fuzzer` to substitute a marker (e.g. `FUZZ`) across a raw request template using a small wordlist.
...
### 6. Scanner Automation & Background Jobs
1. Start passive audits or bounded active scans using `burp_scan_start`.
...
4. Page Scanner findings with `burp_scan_issues`, detail issue evidence with `burp_scan_issue_detail`, and generate HTML/XML reports with `burp_scanner_generate_report`.
