...
[Logger](https://portswigger.net/burp/documentation/desktop/tools/logger) records HTTP generated by Burp tools, Scanner, session handling, and extensions. Use `burp_logger` action `query` to inspect traffic across all sources (Proxy, Repeater, Scanner, Intruder, Extensions) and `burp_logger` action `detail` for full payloads (both bounded to 4096 bytes by default with truncation metadata).
