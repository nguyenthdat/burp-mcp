...
[Logger](https://portswigger.net/burp/documentation/desktop/tools/logger) records HTTP generated by Burp tools, Scanner, session handling, and extensions. Use `burp_logger` action `query` to inspect traffic across all sources (Proxy, Repeater, Scanner, Intruder, Extensions) with compact metadata by default (`include_bodies: false`, `max_body_length: 4096` default), and `burp_logger` action `detail` for individual entries (metadata-only by default, opt into raw bodies via `include_bodies: true` capped at 4096 bytes or bounded projections via `headers_only`/`extract_css`/`extract_json`). Projections use Pest-parsed AST evaluators (`extract_css` with structural/logical pseudos, sibling/child combinators, and `:contains`; `extract_json` with negative indexes, slices, recursive descent, and filter expressions).

[Organizer](https://portswigger.net/burp/documentation/desktop/tools/organizer) allows triage and tracking of interesting HTTP requests/responses. Use `burp_organizer` action `add` to save annotated requests/responses to Organizer and `burp_organizer` action `list` to review saved items.

## Response Diffing & Comparer

Use `burp_diff` action `diff_responses` to compare two HTTP responses (from strings or proxy/logger indexes) and calculate:
...
Use `burp_diff` action `compare_exchanges` to send two raw HTTP payloads directly to Burp's desktop Comparer tab for visual inspection.

## Race Conditions & Single-Packet Attack

Standard multi-threading is often insufficient for tight race conditions due to TCP network jitter. Use `burp_fuzzer` action `race` with `single_packet_attack: true`:
...
Use `burp_fuzzer` action `fuzz` with `attack_mode`:
...
Burp MCP's `burp_scanner` interface supports passive audits, active scans (`start_audit`), and crawls (`start_crawl`). Other bounded crawl/audit features depend on the connected edition and advertised capabilities. Never assume an active scan is safe because it is automated.
For permitted scans:
...
1. Generate distinct payloads with `burp_collaborator` action `generate` and assign one
...
3. Poll at a paced interval with `burp_collaborator` action `poll` (`{limit, cursor?}`).
...
string literals. Use a bounded `burp_settings` action `register_proxy_rule`
...
3. Burp MCP **sitegraph** is a separate optional Rust/SQLite metadata graph. It neither means nor automatically requests `/sitemap.xml`. Read [the dedicated sitegraph reference](./sitegraph.md) before enabling it. The embedded `2026.09.05` pack has 105 standard regex rules evaluated by surface-partitioned byte-safe `RegexSet` matching without absence inference (`missing_content_type_options` detects only explicitly insecure header values `none|0|false|off`). When enabled, `sitegraph_sync` automatically annotates interesting newly indexed HTTP Proxy history entries (medium+ findings, capped at 50/run, stable Burp history ID, red/orange highlights, note preservation with `[SiteGraph]` marker, non-fatal post-commit, HTTP only).
