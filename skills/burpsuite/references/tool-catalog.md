...
| Tool | Input | Purpose |
| `sitegraph` | `{action, url_prefix?, query?, id?, from_id?, to_id?, limit?, cursor?, max_depth?, since?, profile?, format?, snapshot_id?, view_name?, spec_content?}` | SiteGraph attack surface graph analyzer (`status`, `stats`, `sync`, `search`, `security_view`, `import_spec`, `neighbors`, `trace`, `shortest_path`, `clusters`, `impact`, `diff`, `export`, `history_search`, `endpoint_detail`, `projects`, `config`). Enrichment rules load from a Pest-parsed DSL (`.rules`); legacy JSON rule definitions are rejected. In-memory pattern matching executes via byte-safe `RegexSet`. `sync` automatically annotates interesting newly indexed HTTP Proxy history entries (stable Burp history ID, max 50 entries/run, medium+ findings, critical/high -> RED, medium -> ORANGE, low -> YELLOW; preserves existing notes and non-NONE highlights; idempotent `[SiteGraph]` marker; non-fatal post-commit; HTTP only, never WebSockets). |
