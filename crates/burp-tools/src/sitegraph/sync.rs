...
use ::sitegraph::{IssueObservation, SiteGraph, SitemapObservation, SyncBatch, SyncContext, SyncSummary, WebSocketObservation};
use std::sync::Arc;
...
impl SiteGraphSynchronizer {
    ...
    pub(crate) async fn run(&self, prefix: String) -> Result<SyncSummary, String> {
        let graph_id = self
            ...
            .graph_id;
        let scope = if prefix.is_empty() { "all".to_owned() } else { prefix.clone() };
        let checkpoint = self
            .graph
            .checkpoint("burp_sitemap", &scope)
            .await
            .map_err(|error| error.to_string())?;
        let resumed = checkpoint.as_ref().filter(|(_, coverage)| !coverage.complete);
        let mut cursor = resumed
            .and_then(|(_, coverage)| coverage.last_cursor.clone())
            .unwrap_or_default();
        let run_id = resumed.map_or_else(
            || blake3::hash(format!("{graph_id}\\0{scope}\\0{}", time::OffsetDateTime::now_utc().unix_timestamp_nanos()).as_bytes()).to_hex().to_string(),
        ...
        loop {
            let response = self.client.sitemap_snapshot(SitemapSnapshotRequest {
                url_prefix: prefix.clone(),
                page: Some(PageRequest { limit: PAGE_SIZE, cursor: cursor.clone() }),
            }).await.map_err(|error| error.to_string())?;
            let page = response.page.unwrap_or_default();
            let sitemap = response.items.into_iter().map(|entry| SitemapObservation {
                url: entry.url,
                method: entry.method,
                status: entry.status,
            ...
            last_summary = Some(self.graph.sync_with_context(
                &SyncBatch { sitemap, ..SyncBatch::default() },
                &context,
            ).await.map_err(|error| error.to_string())?);
            if end_of_source {
                break;
        ...
        if !issues.is_empty() {
            ...
            last_summary = Some(self.graph.sync_with_context(
                &SyncBatch { issues, ..SyncBatch::default() },
                &context,
            ).await.map_err(|error| error.to_string())?);
        }
        let mut websocket_cursor = String::new();
        loop {
            let response = self.client.proxy_websocket_history(ProxyWebSocketHistoryRequest {
                page: Some(PageRequest { limit: PAGE_SIZE, cursor: websocket_cursor.clone() }),
            }).await.map_err(|error| error.to_string())?;
            let page = response.page.unwrap_or_default();
            let websocket_messages = response.items.into_iter().map(|entry| WebSocketObservation {
            ...
            if !websocket_messages.is_empty() {
                ...
                last_summary = Some(self.graph.sync_with_context(
                    &SyncBatch { websocket_messages, ..SyncBatch::default() },
                    &context,
                ).await.map_err(|error| error.to_string())?);
            }
            if !page.truncated || page.next_cursor.is_empty() { break; }
