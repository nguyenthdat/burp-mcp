...
impl BurpTools {
    ...
    #[tool(
        ...
        description = "Verify IDOR with two authorization contexts. Evidence requires successful 2xx responses; denied/error bodies never confirm. Inputs are bounded HTTP(S) requests.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Audit CORS using response headers only; test_origins is bounded and transport/no-response failures are reported.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Run a bounded role-by-endpoint access matrix. Successful unauthenticated access is reported as a potential violation; failed probes are errors.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        name = "burp_audit_jwt",
        description = "Audit exactly three-segment JWTs with bounded malicious probes; successful HTTP responses are recorded with status and length.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Verify SSRF with bounded Collaborator probes. Interactions count only when their payload matches one generated by this run; use header:NAME, param:NAME, body, or a bare body parameter name.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Run bounded differential and timing SQLi probes. param_type is query (default) or body; existing query parameters are preserved.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Audit GraphQL with structural JSON evidence for introspection, field suggestions, and exactly three-request array batching.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Probe a state-changing endpoint, inspect the named Set-Cookie SameSite attribute, and generate an escaped PoC. No response or denied response is not a vulnerability confirmation.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
    ...
    #[tool(
        ...
        description = "Run bounded OpenAPI/Swagger fuzzing and report completed requests plus HTTP 5xx anomalies; transport and no-response failures stop with an actionable error.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = false,
...
#[tool_handler(router = Self::burp_router(), name = "burp-mcp", version = "3.3.0")]
...
fn mcp_server_info() -> rmcp::model::ServerInfo {
    ...
    .with_server_info(rmcp::model::Implementation::new("burp-mcp", "3.3.0"))
