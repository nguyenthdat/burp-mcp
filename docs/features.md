...
| Tool | Purpose | Preliminary live check |
| `sitegraph` | SiteGraph attack surface graph analyzer (`status`, `stats`, `sync`, `search`, `security_view`, `import_spec`, `neighbors`, `trace`, `shortest_path`, `clusters`, `impact`, `diff`, `export`, `history_search`, `endpoint_detail`, `projects`, `config`). Enrichment rules use a Pest-parsed DSL (`.rules`); legacy JSON rule definitions are rejected, and pattern matching executes via byte-safe `RegexSet`. `sync` automatically annotates interesting newly fetched HTTP Proxy history entries (stable Burp history ID lookup, bounded to max 50 entries per run, medium/high/critical severity threshold, critical/high -> RED, medium -> ORANGE, low -> YELLOW; preserves existing operator notes and non-NONE highlights; idempotent `[SiteGraph]` marker line; non-fatal post-commit execution; HTTP-only, never WebSockets). | Enable with `--enable-sitegraph`, sync site map, and run graph traversals. |
