...
- **Actions**:
  ...
  - `detail`: Get request/response details for a specific Proxy history index. Metadata-only by default (`include_bodies: false`); `include_bodies: true` includes raw Base64 and body text capped at `max_body_length` (default 4096 bytes). Projection options (`headers_only`, `extract_css`, `extract_json`) return bounded projections without raw bodies:
    - `extract_css`: Evaluates HTML responses using a bounded custom Pest grammar and tokenizer. Supports type/universal selectors, classes, IDs, attribute matchers (`=`, `~=`, `|=`, `^=`, `$=`, `*=`) with `i`/`s` case flags and quoted/unquoted values, combinators (child `>`, descendant whitespace, next-sibling `+`, subsequent-sibling `~`), selector lists (`,`), and structural pseudos (`:first-child`, `:last-child`, `:only-child`, `:nth-child(odd|even|An+B)`), logical pseudos (`:not()`), and deliberate nonstandard extensions (`:contains("text")`). Does not claim full CSS4 support.
    - `extract_json`: Evaluates JSON responses using a custom Pest grammar and evaluator. Supports root `$` and `@`, dot and bracket child navigation (`.key`, `['key']`, `["key"]`), quoted escaped property names, wildcards (`.*`, `[*]`), array indexes and negative indexes, full positive/negative slices (`[start:end:step]`, where `step=0` returns empty per RFC 9535), union selectors (`[a, b, 0:2]`), recursive descent (`..key`, `..*`), and filter expressions (`[?(@.price < 10 && !@.archived)]`) with logical precedence, comparisons, `match()`, and `search()`. Parsing has input/depth guards; serialized projection output remains bounded by `max_body_length`.
    - Emits original byte lengths and truncation metadata.
...
| Tool | Purpose | Preliminary live check |
| `sitegraph` | SiteGraph attack surface graph analyzer (`status`, `stats`, `sync`, `search`, `security_view`, `import_spec`, `neighbors`, `trace`, `shortest_path`, `clusters`, `impact`, `diff`, `export`, `history_search`, `endpoint_detail`, `projects`, `config`). The embedded `2026.09.05` typed TOML pack contains 105 standard regex rules evaluated by surface-partitioned byte-safe `RegexSet` matching, persisting exact present spans in finding metadata. No absence inference: `missing_content_type_options` detects only explicitly insecure header values (`none|0|false|off`). Legacy Pest DSL and JSON rule definitions are rejected without dual-format compatibility. `sync` automatically annotates interesting newly fetched HTTP Proxy history entries (stable Burp history ID lookup, bounded to max 50 entries per run, medium/high/critical severity threshold, critical/high -> RED, medium -> ORANGE, low -> YELLOW; preserves existing operator notes and non-NONE highlights; idempotent `[SiteGraph]` marker line; non-fatal post-commit; HTTP only). | Inspect `status` before `sync`; use metadata-only views first. |
